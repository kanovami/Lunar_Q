...
def log_prior(theta):
    mu, eta, alpha, zeta, Delta, eta_gb, d_grain, rho_mantle, rho_core, r_core = theta
    mu_cond = 60 < mu < 90
    eta_cond = 15 < eta < 30
    alpha_cond = 0.1 < alpha < 0.4
    zeta_cond = -5 < zeta < 5
    Delta_cond = -2 < Delta < 1
    eta_gb_cond = 0 < eta_gb < eta
    d_grain_cond = -6 < d_grain < -2
    ...
    if (mu_cond and eta_cond and alpha_cond and zeta_cond and Delta_cond 
        and eta_gb_cond and d_grain_cond and rhom_cond and rhoc_cond and Rc_cond):
        return 0.0
    return -np.inf
...
nwalkers = 32
initial  = [80, 21, 0.2, 0, -1, 8, -4, 3.5, 5, 330]
theta = initial
...
labels = [r"$\mu_{\rm{m}}$", r"$\log\;\eta_{\rm{m}}$", r"$\alpha$", r"$\zeta$", r"$\log\;\Delta$", 
          r"$\log\;\eta_{\rm{gb}}$", r"$\log\; d_{\rm{grain}}$", r"$\rho_{\rm{m}}$", r"$\rho_{\rm{c}}$", r"$R_{\rm{c}}$"]
