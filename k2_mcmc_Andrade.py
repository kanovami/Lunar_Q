...
def k2(mu, eta, alpha, zeta, rho_mantle, rho_core, r_core, omg):
    
    # Allocate Fortran arrays
    # Planet interior structure - layerwise, from core to surface
    #-------------------------------------------------------------
    mconst.ire  = [0, 1, 0]               # rheological models
    mconst.rho  = [rho_core, rho_mantle, 2550]         # densities
    mconst.arad = [r_core, r_planet-40e3, r_planet]# radii of interfaces
...
def log_likelihood(theta, xobs, sig):
    ...
    k_Love_m, h_Love_m, k_Love3_m = k2(
                ...
                theta[4]*1e3, theta[5]*1e3, theta[6]*1e3, omg_month)
    k_Love_a, h_Love_a, k_Love3_a = k2(
                theta[0]*1e9, 10**theta[1], theta[2], 10**theta[3],
                theta[4]*1e3, theta[5]*1e3, theta[6]*1e3, omg_year)
...
labels = [r"$\mu_{\rm{m}}$", r"$\log\;\eta_{\rm{m}}$", r"$\alpha$", r"$\zeta$", 
          r"$\rho_{\rm{m}}$", r"$\rho_{\rm{c}}$", r"$R_{\rm{c}}$"]
