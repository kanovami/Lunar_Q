!==========================================================================
! Code calculating volumetric heating (M. Walterova, 2019)
!==========================================================================
...
REAL(DP) :: rej,imj,al,f220,f201,b22,b20,dheat
REAL(DP) :: fact
...
il = 2 ! do not calculate heating in the core (start with the second layer)

!======================================================
! Calculate and output the volumetric heating (W/m**3)
!======================================================
OPEN(ifile,file='Moon_sc_rad.dat')

phi = 0._DP
do ir=1,nr
  rad=arad(il-1)+(ir-1)*(arad(il)-arad(il-1))/(nr-1)
  do il2=il,nl
    if ((rad.ge.arad(il2-1)).and.(rad.le.arad(il2))) exit
  end do
  do it=1,nth
    ...
    !  write(ifile, '(3g20.10)') phi,thet,dheat
    !end do
    call unit_heating(dheat,rad,thet,phi,il2)
    write(ifile, '(3g20.10)') rad/rpl,thet,dheat
  end do
end do
...
DO i=0,nhans
  ...
  DO j=0,nhans
    ...
    !print *,freq(j+1),dsin(4*phi+lag(j+1)),delta_sin(arg),arg,dsin(arg*pi)
    !PAUSE
