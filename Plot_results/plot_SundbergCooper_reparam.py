...
# Tidal Love numbers:
# -------------------
def k2(mu, eta, alpha, zeta, Delta, eta_gb, d_grain, rho_mantle, rho_core, rho_crust, 
       r_core, d_crust, omg):
    
    delta_gb = 1e-9
    trel = (eta_gb/eta)*(d_grain/delta_gb)
...
def log_probability(theta, xobs, sig):
    ...
    k_Love_m, h_Love_m, k_Love3_m = k2(
                ...
                10**theta[4], 10**theta[5], 10**theta[6], theta[7]*1e3, theta[8]*1e3,
                2550, theta[9]*1e3, 40e3, omg_month)
    k_Love_a, h_Love_a, k_Love3_a = k2(
                theta[0]*1e9, 10**theta[1], theta[2], 10**theta[3],
                10**theta[4], 10**theta[5], 10**theta[6], theta[7]*1e3, theta[8]*1e3,
                2550, theta[9]*1e3, 40e3, omg_year)
    ...
    k2Q_a = -k_Love_a.imag
    MoIFmod = MoIF(theta[7]*1e3, theta[8]*1e3, 2550, theta[9]*1e3, 40e3)
    massmod = mass(theta[7]*1e3, theta[8]*1e3, 2550, theta[9]*1e3, 40e3)
...
# Define file name
file_name = './Sundberg-Cooper_reparameterised/chain_core_REAL.dat'
...
with open('SundbergCooper_reparam-best_fits.csv', 'w', newline='') as f:
    writer = csv.writer(f, quoting=csv.QUOTE_NONNUMERIC)
    writer.writerow(["chi2","mu","eta","alpha","zeta","Delta","eta_gb", "d_grain",
...
#=============================
figure = corner.corner(df[burnin::2,:7],
    labels = [r"$\mu_{\rm{m}}\; \left[\rm{GPa}\right]$", 
              r"$\log\;\eta_{\rm{m}}\; \left[\rm{Pa\; s}\right]$", 
              r"$\alpha$", r"$\log\;\zeta$", r"$\log\;\Delta$", 
              r"$\log\;\eta_{\rm{gb}}\; \left[\rm{Pa\; s}\right]$",
              r"$\log\; d_{\rm{grain}}\; \left[\rm{m}\right]$"],
    smooth=True,
    color='indianred',
...
plt.savefig('SundbergCooper_reparam-RHEO.png', dpi=300, bbox_inches = 'tight')
...
plt.savefig('SundbergCooper_reparam-GEO.png', dpi=300, bbox_inches = 'tight')
...
with open('SundbergCooper_reparam-random100.csv', 'w', newline='') as f:
    writer = csv.writer(f, quoting=csv.QUOTE_NONNUMERIC)
    writer.writerow(["chi2","mu","eta","alpha","zeta","Delta","eta_gb","d_grain",
                     "rho_mantle","rho_core","R_core"])
    for i in range(100):
...
for theta in samples:
    ...
    for omg in freqs:
        k_Love, h_Love, k_Love3 = k2(
            theta[0]*1e9, 10**theta[1], theta[2], 10**theta[3],
            10**theta[4], 10**theta[5], 10**theta[6], theta[7], theta[8],
            2550, theta[9]*1e3, 40e3, omg)
        k2s.append(k_Love)
...
plt.savefig('./SundbergCooper_reparam-OVERVIEW.png', dpi=200, bbox_inches = 'tight')